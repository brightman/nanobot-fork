...
### 3) Fork update timeline

This section tracks fork-specific changes in reverse chronological order.

#### 2026-03-03

- Added runtime traceability for main agent + subagent:
  - Trace storage: `~/.nanobot/workspace/traces/YYYY-MM-DD/<trace_id>.jsonl`
  - Trace spans/events include: `context.build`, `agent.turn`, `llm.call`, `tool.exec`, subagent routing/execution
  - End contract recorded as `FINAL_DONE` / `BLOCKED`
- Added trace CLI:
  - `nanobot trace latest -n 10`
  - `nanobot trace show <trace_id>`
  - `nanobot trace errors`
  - `nanobot trace stuck --threshold 60`

#### 2026-03-02

- Subagent system aligned to Claude-style workflow:
  - Subagent profiles loaded from `~/.nanobot/agents/*.md` and `<workspace>/agents/*.md`
  - Main agent semantic routing: `main` vs `subagent`
  - Subagent frontmatter supports `spawn`, `skills`, `mcp`, `maxTurns`, `model`
- CLI updated to interactive selector mode:
  - `nanobot agents` lists profiles, select by name/number, then continuous chat

#### 2026-03-01

- Per-user memory isolation added:
  - `memory/users/<user_key>/MEMORY.md`
  - `memory/users/<user_key>/HISTORY.md`
- SDR scenario packaged under `usercase/workspace-sdr/`
- Structured SDR profile fields added:
  - `company`, `role`, `pains`, `budget_signal`, `timeline`, `objections`, `next_step`

#### 2026-02-28

- Channel/provider compatibility upgrades:
  - DingTalk inbound voice message handling (with fail-open behavior)
  - Tool name normalization for Bedrock/OpenRouter strict validation (`[a-zA-Z0-9_-]`)
